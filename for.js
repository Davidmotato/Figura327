var estadoCivil=0, edad=0, estatura=0, genero=0, contadorPersonas=0, acumuladorEstatura=0, acumuladorEdad=0, promedioEstatura=0, porcentajeCondicion, totalPersonas=0;
...
for (let numeroPersonas = 1; numeroPersonas <=totalPersonas; numeroPersonas++){
    ...
    estatura=parseFloat(prompt("Ingrese la estatura de la persona " +numeroPersonas + "en centimetros"));
    genero=parseInt(prompt("Ingrese el genero de la persona " +numeroPersonas + "1.Maculino 2.Femenino"));
    estadoCivil=parseInt(prompt("Ingrese el estado civil de la persona " +numeroPersonas + "1.Soltera 2.Casada"));

    if((edad>=18) && (estatura>1.70) && (genero==1) && (estadoCivil==1)) {
